        ...
        switch (inst.op) {
          ...
          case INST(OP): {
            ...
            for (int i = 0; i < node->inputs().size(); i++) {
              ...
              auto& input = peek(stack, i, node->inputs().size());
              if (input.isTensor()) {
                auto& t = input.toTensor();
                if (t.has_storage()) {
                  log << i << " (" << t.storage().use_count() << "), ";
                } else {
                  log << i << " (" << -1 << "), ";
                }
              } else {
            ...
            for (int i = 0; i < node->outputs().size(); i++) {
              ...
              auto& input = peek(stack, i, node->outputs().size());
              if (input.isTensor()) {
                auto& t = input.toTensor();
                if (t.has_storage()) {
                  log << i << "(" << t.storage().use_count() << "), ";
