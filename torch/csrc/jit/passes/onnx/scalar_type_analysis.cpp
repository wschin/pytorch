...
static c10::optional<c10::ScalarType> PromoteScalarTypesWithCategory(
  ...
  if (typeFromTensor.has_value())
    std::cout << "[PromoteScalarTypesWithCategory] type from tensor: " << typeFromTensor.value() << std::endl;
  else
    std::cout << "[PromoteScalarTypesWithCategory] type from tensor: " << "None" << std::endl;
  if (typeFromScalar.has_value())
    std::cout << "[PromoteScalarTypesWithCategory] type from scalar: " << typeFromScalar.value() << std::endl;
  else
    std::cout << "[PromoteScalarTypesWithCategory] type from scalar: " << "None" << std::endl;
...
static c10::optional<c10::ScalarType> InferExpectedScalarType(const Node* n) {
  ...
  std::for_each(
      n->inputs().begin(), n->inputs().end(), [&](const Value* input) {
        ...
        std::cout << "[InferExpectedScalarType] input->node(): " << input->node() << std::endl;
        if (nkind == onnx::Gather &&
          ...
          std::cout << "[InferExpectedScalarType] input->type(): " << input->type() << std::endl;
          typesFromTensors.emplace_back(*scalar_type);
        }
      });
  ...
  for (auto t : typesFromTensors) {
    std::cout << "[InferExpectedScalarType] type from tensor: " << t << std::endl;
  }
  for (auto t : typesFromScalars) {
    std::cout << "[InferExpectedScalarType] type from scalar: " << t << std::endl;
  }
  if (output_st.has_value()) {
    std::cout << "[InferExpectedScalarType] type from output: " << output_st.value() << std::endl;
  } else {
    std::cout << "[InferExpectedScalarType] type from output: " << "None" << std::endl;
  }
  ...
  if (st.has_value()) {
    std::cout << "[InferExpectedScalarType] inferred scalar type (st): " << st.value() << std::endl;
  } else {
    std::cout << "[InferExpectedScalarType] failed to infer scalar type" << std::endl;
  }
  return st;
}
...
static void ImplicitCastNodeForONNX(Node* n) {
  if (IsImplicitCastSupported(n->kind())) {
    ...
    if (expected_scalar_type) {
      std::cout << "[scalar_type_analysis.cpp] " << *n << "\n expected_scalar_type: " << expected_scalar_type.value() << std::endl;
