...
struct MyGraphFuser {
  using FusionCallback = std::function<bool(MyGraphFuser*, Node*)>;
  ...
  void run() {
    ...
    for (Node* node : block_->nodes()) {
      for (Block* sub_block : node->blocks()) {
        MyGraphFuser(aliasDb_, sub_block, callback_, kind_, strict_fuser_check_)
            .run();
      }
    }
  }
...
void MyFuseGraph(
  ...
  auto g = MyGraphFuser(
      &db,
      graph->block(),
      [=](MyGraphFuser* gf, Node* n) { return fn(n) || n->kind() == kind; },
